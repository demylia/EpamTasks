...
-- --------------------------------------------------
-- Date Created: 05/30/2015 08:40:53
...
GO
IF OBJECT_ID(N'[dbo].[FK_CameraQueue]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[QueueSet] DROP CONSTRAINT [FK_CameraQueue];
GO
...
-- Creating foreign key on [CameraId] in table 'QueueSet'
ALTER TABLE [dbo].[QueueSet]
ADD CONSTRAINT [FK_CameraQueue]
    FOREIGN KEY ([CameraId])
    REFERENCES [dbo].[Camera]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_CameraQueue'
CREATE INDEX [IX_FK_CameraQueue]
ON [dbo].[QueueSet]
    ([CameraId]);
GO
